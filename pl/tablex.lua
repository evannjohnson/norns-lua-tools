...
local types = require ('tools.pl.types')
...
local function setmeta (res,tbl,pl_class)
    local mt = getmetatable(tbl) or pl_class and require('tools.pl.' .. pl_class)
    return mt and setmetatable(res, mt) or res
end

local function makelist(l)
    return setmetatable(l, require('tools.pl.List'))
end

local function makemap(m)
    return setmetatable(m, require('tools.pl.Map'))
end
...
function tablex.makeset (t)
    assert_arg_indexable(1,t)
    return setmetatable(tablex.pairmap(set_op,t),require('tools.pl.Set'))
end
